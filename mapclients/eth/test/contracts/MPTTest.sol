...
contract MPTTest is ILightNodePoint {
    ...
    {
        bytes memory node = data.proof[data.proofIndex];
        console.logUint(111111111111111111112);
        console.logUint(data.proofIndex);
        console.logUint(111111111111111111110);
        console.logUint(data.keyIndex);
        console.logUint(data.key.length);
        console.logUint(1111111111111111111100);
        ...
        if (data.keyIndex >= data.key.length) {
            ...
            console.logUint(data.keyIndex);
            console.logBytes(data.key);
            uint256 index = uint256(uint8(data.key[data.keyIndex]));
            console.logUint(index);
            console.logUint(RLPReader.toRlpItem(node).toList().length);
    ...
    function b2b32(bytes memory data)  internal returns(bytes32 part) {
        assembly {
    ...
    function getNibbles(bytes1 b) internal  returns (bytes1 nibble1, bytes1 nibble2) {
        assembly {
            nibble1 := shr(4, b)
    ...
    function expandKeyEven(bytes memory data) internal  returns (bytes memory) {
    ...
    function expandKeyOdd(bytes memory data) internal  returns(bytes memory) {
    ...
    function getVerifyTrieProof2(MerkleProof memory _receiptProof)
    public  {
        bool result =  verifyTrieProof(_receiptProof);
