...
impl Receipt {
    pub fn encode_index(&self) -> Vec<u8> {
        if let 0 = self.receipt_type.0 {
            return rlp::encode(self);
        }

        let mut res: Vec<u8> = Vec::new();
        res.push(self.receipt_type.0 as _);
...
impl Encodable for Receipt {
    fn rlp_append(&self, s: &mut RlpStream) {
        ...
        // cumulative_gas_used
        s.append(&self.cumulative_gas_used.0);
...
mod tests {
    ...
    fn test_serde_json_receipt() {
        let header = Header{
            ...
            extra: vec![1,2,3],
            min_digest: Hash::from_bytes(&hex::decode("7285abd5b24742f184ad676e31f6054663b3529bc35ea2fcad8a3e0f642a46f7").unwrap()).unwrap().to_owned(),
