...
use crate::alt_bn128::{g1_sum, g1_multiexp, pairing_check, InvalidInput, split_elements};

pub fn alt_bn128_g1_sum(data: Vec<u8>) -> Result<[u8; 64], InvalidInput> {
    let elements = split_elements(&data)?;

    g1_sum(elements)
}

pub fn alt_bn128_g1_multiexp(data: Vec<u8>) -> Result<[u8; 64], InvalidInput> {
    let elements = split_elements(&data)?;

    g1_multiexp(elements)
}

pub fn alt_bn128_pairing_check(data: Vec<u8>) -> Result<bool, InvalidInput> {
    let elements = split_elements(&data)?;

    pairing_check(elements)
}
...
#[cfg(test)]
mod tests {
    use crate::{alt_bn128_g1_multiexp, alt_bn128_g1_sum, alt_bn128_pairing_check, InvalidInput};

    #[test]
    fn test_alt_bn128_g1_sum() {
        #[track_caller]
        fn check(input: &[u8], expected: Result<&[u8], &str>) {
            let res = alt_bn128_g1_sum(input.to_vec());
            if let Some((actual, expected)) = check_result(res, expected) {
    ...
    fn test_alt_bn128_g1_multiexp() {
        #[track_caller]
        fn check(input: &[u8], expected: Result<&[u8], &str>) {
            let res = alt_bn128_g1_multiexp(input.to_vec());
            if let Some((actual, expected)) = check_result(res, expected) {
                assert_eq!(&actual, expected)
            }
        }
    ...
    fn test_alt_bn128_pairing_check() {
        ...
        fn check(input: &[u8], expected: Result<u64, &str>) {
            let res = alt_bn128_pairing_check(input.to_vec());
