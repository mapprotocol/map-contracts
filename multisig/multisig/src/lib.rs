...
pub struct MultiSigContract {
    ...
    /// All confirmed requests which have not been executed.
    schedule: UnorderedMap<RequestId, u64>,
    /// Request lock period (time before a request can be executed)
    request_lock: u64,
}
...
impl MultiSigContract {
    ...
    pub fn confirm(&mut self, request_id: RequestId) -> PromiseOrValue<bool> {
        ...
        if confirmations.len() as u32 + 1 >= self.num_confirmations {
            let request_with_signer = self
                ...
                .get(&request_id)
                .unwrap_or_else(|| env::panic_str("No such request"));
            let min_exe_time = request_with_signer.added_timestamp + self.request_lock;
            let cur_timestamp = env::block_timestamp();
            if cur_timestamp < min_exe_time {
                log!("Request {} cannot be executed before time {}, current block time: {}, adding to schedule queue", request_id, min_exe_time, cur_timestamp);
                self.schedule.insert(&request_id, &min_exe_time);
    ...
    ********************************/
    ...
    fn remove_request(&mut self, request_id: RequestId) -> MultiSigRequest {
        // remove confirmations for this request
        self.confirmations.remove(&request_id);
        // remove request id from schedule queue
        self.schedule.remove(&request_id);
...
mod tests {
    ...
    const REQUEST_LOCK: u64 = 900_000_000_000;
    ...
    const NON_MEMBER_KEY: &str = "ed25519:9FTPoigCHjSeP9D8scEBayUUZJLaHgaigd3wNuucpzr8";
    ...
    fn context_with_account_key_timestamp(
        account_id: AccountId,
        key: PublicKey,
        timestamp: u64,
        amount: Balance,
    ...
    fn context_with_key_future(key: PublicKey, amount: Balance) -> VMContext {
        VMContextBuilder::new()
            ...
            .predecessor_account_id(contract())
            .signer_account_id(contract())
            .signer_account_pk(key)
            .account_balance(amount)
            .build()
    }
    ...
    #[test]
    fn test_multi_3_of_n_time_lock_001() {
        let amount = 1_000;
        testing_env!(context_with_key(
            PublicKey::from(
                "Eg2jtsiMrprn7zgKKUk79qM1hWhANsFyE6JSX4txLEuy"
        ...
        c.confirm(request_id);
        assert_eq!(c.confirmations.get(&request_id).unwrap().len(), 3);
        assert_eq!(c.get_confirmations(request_id).len(), 3);
        assert_eq!(c.requests.len(), 1);
        ...
        assert_eq!(c.requests.len(), 0);
        assert_eq!(c.schedule.len(), 0);
        assert!(c.confirmations.get(&request_id).is_none());
    }

    #[test]
    fn test_multi_3_of_n_time_lock_002() {
        let amount = 1_000;
        testing_env!(context_with_key(
            PublicKey::from(
                "Eg2jtsiMrprn7zgKKUk79qM1hWhANsFyE6JSX4txLEuy"
    ...
    fn add_key_delete_key_storage_cleared() {
        ...
        let request_id = c.add_request_and_confirm(request3.clone());
        assert_eq!(c.requests.len(), 2);
        assert_eq!(c.get_num_requests_per_member(new_member.clone()), 2);
