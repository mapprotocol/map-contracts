...
const backoff = (retries, fn, delay = DELAY, wait = BACKOFF) =>
    fn().catch((err) =>
        retries > 1 ? sleep(delay).then(() => backoff(retries - 1, fn, delay * wait)) : Promise.reject(err)
    );
...
class RobustWeb3 {
    ...
    async maxPriorityFeePerGas() {
        ...
        try {
            ...
            value = this.web3.utils.toBN(gasPrice).sub(this.web3.utils.toBN(baseFeePerGas)).toString();
        }
    ...
    async getFeeData(multiplier = 1) {
        ...
        const maxPriorityFeePerGas = this.web3.utils
            .toBN(await this.maxPriorityFeePerGas())
        ...
        const maxFeePerGas = this.web3.utils.toBN(2).mul(baseFeePerGas).add(maxPriorityFeePerGas);
        ...
        while (gasPrice < 10000 * 1e9) {
            try {
                ...
                const tx = {
                    ...
                    gasPrice: options.gasPrice ? options.gasPrice : Web3.utils.toHex(gasPrice),
...
async function nearJsonContractFunctionCall(contractId, sender, method, args, gas, amount) {
    ...
    return await signAndSendTransaction(accessKey, sender, contractId, [
        nearAPI.transactions.functionCall(method, Buffer.from(JSON.stringify(args)), gas, amount),
    ]);
}
