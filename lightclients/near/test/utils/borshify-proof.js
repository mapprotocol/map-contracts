...
function borshifyOutcomeProof(proof) {
    const statusToBuffer = (status) => {
        if ("SuccessValue" in status) {
            ...
            return Buffer.concat([Buffer.from([2]), Web3.utils.toBN(data.length).toBuffer("le", 4), data]);
        } else if ("SuccessReceiptId" in status) {
            return Buffer.concat([Buffer.from([3]), bs58.decode(status.SuccessReceiptId)]);
        } else {
    ...
    return Buffer.concat([
        ...
        Buffer.concat(
            proof.outcome_proof.proof.map((p) =>
                Buffer.concat([bs58.decode(p.hash), Buffer.from([p.direction === "Right" ? 1 : 0])])
            )
        ),
        ...
        Buffer.concat([
            ...
            Buffer.concat(
                proof.outcome_proof.outcome.logs.map((log) =>
                    Buffer.concat([Web3.utils.toBN(log.length).toBuffer("le", 4), Buffer.from(log, "utf8")])
                )
            ),
            ...
            Web3.utils
                .toBN(
                    proof.block_header_lite.inner_lite.timestamp_nanosec || proof.block_header_lite.inner_lite.timestamp
                )
                .toBuffer("le", 8),
            ...
            Buffer.concat(
                proof.block_proof.map((bp) =>
                    Buffer.concat([bs58.decode(bp.hash), Buffer.from([bp.direction === "Right" ? 1 : 0])])
                )
            ),
        ]),
    ]);
