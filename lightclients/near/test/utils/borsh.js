...
const { Web3, nearAPI, sleep, backoff, signAndSendTransaction } = require("./robust");
...
const txnStatus = async (account, txHash, retries = RETRY_TX_STATUS, wait = 1000) => {
    ...
    const flatLogs = [result.transaction_outcome, ...result.receipts_outcome].reduce(
        (acc, it) => acc.concat(it.outcome.logs),
        []
    );
    ...
    constructor(borshSchema, account, contractId, options) {
            ...
            Object.defineProperty(this, d.methodName, {
                ...
                value: async (args) => {
                    ...
                    return (
                        ...
                        deserialize(borshSchema, d.outputFieldType, Buffer.from(result.result))
                    );
                },
            });
        });
        options.changeMethods.forEach((d) => {
            Object.defineProperty(this, d.methodName, {
                ...
                value: async (args, gas, amount) => {
                    ...
                    const rawResult = await signAndSendTransaction(this.accessKey, this.account, this.contractId, [
        ...
        options.changeMethods.forEach((d) => {
            Object.defineProperty(this, d.methodName + "_async", {
                ...
                value: async (args, gas, amount) => {
                    args = serialize(borshSchema, d.inputFieldType, args);
                    return await signAndSendTransactionAsync(this.accessKey, this.account, this.contractId, [
                        nearAPI.transactions.functionCall(
                            d.methodName,
                            Buffer.from(args),
                            gas || DEFAULT_FUNC_CALL_AMOUNT,
...
function borshify(block) {
    return Buffer.concat([
        ...
        Buffer.concat([
            ...
            Web3.utils.toBN(block.inner_lite.timestamp_nanosec || block.inner_lite.timestamp).toBuffer("le", 8),
        ...
        Buffer.concat(
            block.next_bps.map((nextBp) =>
                Buffer.concat([
                    Object.prototype.hasOwnProperty.call(nextBp, "validator_stake_struct_version") &&
                    nextBp.validator_stake_struct_version === "V2"
...
const readerToHex = (len) => (reader) => Web3.utils.bytesToHex(reader.readFixedArray(len));
