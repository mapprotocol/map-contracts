...
async function verifyProofData() {
    ...
    const iface = new hre.ethers.utils.Interface(["function initialize(address _owner, bytes[2] memory initDatas)"]);
    ...
    let values = [head, proof];

    await sleep(30000);

    let result = await proxy.verifyProofData(ethers.utils.defaultAbiCoder.encode(types, values), {
        gasLimit: 20000000,
    });
