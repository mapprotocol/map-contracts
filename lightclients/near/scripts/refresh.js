...
async function refreshInitData() {
    ...
    let lastblock = await near.connection.provider.sendJsonRpc("block", last);

    last = {
        block_id: lastblock.header.height - 50,
    };

    lastblock = await near.connection.provider.sendJsonRpc("block", last);

    let pre = {
        block_id: lastblock.header.height - 86010,
    };

    let preblock = await near.connection.provider.sendJsonRpc("block", pre);

    let block = await near.connection.provider.sendJsonRpc("next_light_client_block", [lastblock.header.hash]);

    let validators = await near.connection.provider.sendJsonRpc("next_light_client_block", [preblock.header.hash]);
