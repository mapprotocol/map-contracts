...
contract LightNode is UUPSUpgradeable, Initializable, Pausable, ILightNode {
    ...
    function updateBlockHeader(
        ...
        _lastSyncedBlock += Verify._getEpochNumber(
            chainId,
            _lastSyncedBlock + 1
        );
    ...
    function _removeExcessEpochValidators() internal {
        ...
        if (
            ...
            minValidBlocknum &&
            validators[(remove + 1) / Verify._getEpochNumber(chainId, remove)]
                .length >
            0
        ) {
            minValidBlocknum =
                remove +
                Verify._getEpochNumber(chainId, remove) +
                1;
            delete validators[
                (remove + 1) / Verify._getEpochNumber(chainId, remove)
            ];
        }
    }
