...
describe("LightNode", function () {
    ...
    async function deployFixture() {
        ...
        let initData = LightNode.interface.encodeFunctionData("initialize", [chainId, wallet.address, mPTVerify.address, initBlock, validators]);
    ...
    describe("Deployment", function () {
        ...
        it("updateBlockHeader() -> reverts paused ", async function () {
            ...
            await expect(lightNode.updateBlockHeader(await lightNode.getHeadersBytes(data.addBlock1, data.quorumCert1, data.validators1))).to.be.revertedWith('Pausable: paused');
        ...
        it("updateBlockHeader() -> correct ", async function () {
            ...
            await lightNode.updateBlockHeader(await lightNode.getHeadersBytes(data.addBlock1, data.quorumCert1, data.validators1));

            let current = await lightNode.headerHeight();

            expect(current).to.eq(21729190)

            await lightNode.updateBlockHeader(await lightNode.getHeadersBytes(data.addBlock2, data.quorumCert2, data.validators2));
