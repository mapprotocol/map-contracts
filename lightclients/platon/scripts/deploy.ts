...
async function deploy() {
  ...
  await(await proxy.updateBlockHeader(await lightNode.getHeadersBytes(data.addBlock1, data.quorumCert1, data.validators1))).wait();

  let current = await proxy.headerHeight();

  console.log(current);
  await (await proxy.updateBlockHeader(await lightNode.getHeadersBytes(data.addBlock2, data.quorumCert2, data.validators2))).wait();
