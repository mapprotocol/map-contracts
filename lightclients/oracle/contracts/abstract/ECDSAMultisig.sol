...
abstract contract ECDSAMultisig {
    ...
    function _setQuorum(uint256 quorum) internal {
        ECDSAMultisigStorage.Layout storage l = ECDSAMultisigStorage
            .layout();

        if (quorum > l.signers.length())
            revert ECDSAMultisig_InsufficientSigners();
        l.quorum = quorum;
    }
    ...
    function _addSigner(address account) internal {
        ...
        if (l.signers.length() >= 256)
            revert ECDSAMultisig_SignerLimitReached();
        if (!l.signers.add(account))
            revert ECDSAMultisig_AddSignerFailed();
    }
    ...
    function _removeSigner(address account) internal {
        ...
        if (!l.signers.remove(account))
            revert ECDSAMultisig_RemoveSignerFailed();
    }
    ...
    function _verifySignatures(
        ...
        if (l.quorum > signatures.length)
            revert ECDSAMultisig_QuorumNotReached();
        ...
        unchecked {
            for (uint256 i; i < signatures.length; i++) {
                ...
                address signer = keccak256(
                    abi.encodePacked(root, l.version, blockNum, chainId)
                ...
                if (index >= 256)
                    revert ECDSAMultisig_RecoveredSignerNotAuthorized();
               
                uint256 shift = 1 << index;

                if (signerBitmap & shift != 0)
                    revert ECDSAMultisig_SignerAlreadySigned();
