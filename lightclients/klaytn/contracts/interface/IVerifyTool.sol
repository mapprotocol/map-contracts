...
interface IVerifyTool {
    function bytesToAddressArray(bytes memory data) external pure returns (address[] memory);
    ...
    function checkHeaderParam(IKlaytn.BlockHeader memory header) external view returns (bool);

    function recoverSigner(bytes memory seal, bytes32 hash) external pure returns (address);
