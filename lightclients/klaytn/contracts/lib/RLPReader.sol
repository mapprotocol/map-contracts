...
library RLPReader {
    ...
    function next(Iterator memory self) internal pure returns (RLPItem memory) {
        require(hasNext(self),"not has next");
    ...
    function toAddress(RLPItem memory item) internal pure returns (address) {
        ...
        require(item.len == 21,"size error 21");
    ...
    function toUint(RLPItem memory item) internal pure returns (uint256) {
        require(item.len > 0 && item.len <= 33,"size error 0_33");
    ...
    function toUintStrict(RLPItem memory item) internal pure returns (uint256) {
        // one byte prefix
        require(item.len == 33,"size error 33");
    ...
    function toBytes(RLPItem memory item) internal pure returns (bytes memory) {
        require(item.len > 0,"size is zero");
     ...
     */
    function safeGetItemByIndex(RLPItem memory item, uint idx)
    internal
    pure
    returns (RLPItem memory)
    {
