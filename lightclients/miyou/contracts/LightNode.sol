...
contract LightNode is UUPSUpgradeable, Initializable, Pausable, ILightNode {
    uint256 public constant EPOCH_NUM = 30000;
    ...
    function updateBlockHeader(
        ...
        _lastSyncedBlock += Verify.EPOCH_NUM;

        require( _blockHeaders[0].number == _lastSyncedBlock,"invalid syncing block");

        uint256 epoch = Verify.EPOCH_NUM;

        // index 0 header verify by pre validators others by index 0 getValidators
        validators[_lastSyncedBlock / epoch] = Verify._getValidators(_blockHeaders[0].extraData);
    ...
    function _verifyBlockHeaders(
        ...
        for (uint256 i = 0; i < _blockHeaders.length; i++) {
            if (i == 0) {
                if (
                    !Verify._validateHeader(_blockHeaders[i],minEpochBlockExtraDataLen,_blockHeaders[i])
                ) {
                ...
                if (
                    !Verify._validateHeader(_blockHeaders[i],minEpochBlockExtraDataLen,_blockHeaders[i - 1])
                ) {
                    return (false, "invalid block header");
                }
            }
            ...
            uint256 epoch = Verify.EPOCH_NUM;
			(bool result, uint256 signerIndex) = Verify._containsValidator(validators[_blockHeaders[i].number / epoch],signer);
			if (
			    !result
			) {
			    return (false, "invalid block header signer");
			}
			if(_blockHeaders[i].number % (validators[_blockHeaders[i].number / epoch].length / ADDRESS_LENGTH) == signerIndex) {
				if(_blockHeaders[i].difficulty != 2) {
					return (false, "invalid block difficulty");
				}
			} else {
				if(_blockHeaders[i].difficulty != 1) {
					return (false, "invalid block difficulty");
				}
			}
        }
