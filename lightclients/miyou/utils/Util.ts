...
export async function getBlock(blockNumber: number, provider: JsonRpcProvider) {
    ...
    let blockHeader = new BlockHeader(rpcHeader.parentHash, rpcHeader.sha3Uncles,
		...
		console.log('["'+rpcHeader.parentHash+'","' +rpcHeader.sha3Uncles+'","' +
        rpcHeader.miner+'","' + rpcHeader.stateRoot+'","' + rpcHeader.transactionsRoot+'","' +
        rpcHeader.receiptsRoot+'","' + rpcHeader.logsBloom+'",' + BigNumber.from(rpcHeader.difficulty).toString() +',' + BigNumber.from(rpcHeader.number).toString()+',' +
        BigNumber.from(rpcHeader.gasLimit).toString()+',' +  BigNumber.from(rpcHeader.gasUsed).toString()+',' + BigNumber.from(rpcHeader.timestamp).toString()+',"' + 
        rpcHeader.extraData+'","' +  rpcHeader.mixHash+'","' +  rpcHeader.nonce +'"]')
...
export async function getProof(txHash: string, rpc: string,confirms:number|string) {
    ...
    let receiptProof = new ReceiptProof(txReceipt, proof.key, proof.proof);
...
async function receiptProof(txHash: string, uri: string | undefined) {
    ...
    let receipts = await Promise.all(rpcBlock.transactions.map((siblingTxHash: string) => {
