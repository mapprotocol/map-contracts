...
library RLPReader {
    ...
    function toRlpItem(bytes memory item, uint256 offset) internal pure returns (RLPItem memory) {
        uint256 memPtr;
        assembly {
            memPtr := add(item, add(offset, 0x20))
        }

        return RLPItem(_itemLength(memPtr) , memPtr);
    }
